export default {
  title: "申请领取",
  subtitle: "选择服务器并配置您的虚拟机或容器实例",
  userQuotaInfo: "用户配额信息",
  maxInstances: "最大实例数",
  cpuCoreLimit: "CPU核心限制",
  memoryLimit: "内存限制",
  diskLimit: "硬盘限制",
  trafficLimit: "流量限制",
  cores: "核",
  selectProvider: "选择服务器",
  defaultGroup: "默认分组",
  refresh: "刷新",
  dataRefreshed: "数据已刷新",
  refreshFailed: "刷新失败，请稍后重试",
  location: "位置",
  virtualizationType: "虚拟化",
  networkMode: "网络模式",
  availableInstances: "可用实例",
  container: "容器",
  vm: "虚拟机",
  unlimited: "不限制",
  noProvidersAvailable: "当前没有可用的服务器节点",
  possibleReasons: "可能原因",
  reasonNodeDown: "节点资源未就绪",
  reasonInstanceLimit: "服务器实例数已达上限或不满足",
  reasonNoPermission: "服务器未赋权或检查失败",
  statusActive: "正常",
  statusOffline: "离线",
  statusPartial: "部分可用",
  statusInactive: "未激活",
  noProvidersDescription: "暂无可用服务器节点",
  noProvidersMessage: "当前没有可用的服务器节点",
  noProvidersHint: "可能原因：节点资源未同步、服务器离线或配置不完整",
  configInstance: "配置实例",
  instanceType: "实例类型",
  selectInstanceType: "选择实例类型",
  systemImage: "系统镜像",
  selectSystemImage: "选择系统镜像",
  imageRequirements: "最低要求: {memory}MB内存 / {disk}GB硬盘",
  memorySpec: "内存",
  selectMemorySpec: "选择内存",
  diskSpec: "磁盘",
  selectDiskSpec: "选择磁盘",
  bandwidthSpec: "带宽",
  selectBandwidthSpec: "选择带宽",
  cpuSpec: "CPU",
  selectCpuSpec: "选择CPU",
  remarks: "备注说明",
  remarksPlaceholder: "请输入备注说明（可选）",
  submitApplication: "提交申请",
  submitInProgress: "正在提交申请，请勿重复点击...",
  pleaseSelectInstanceType: "请选择实例类型",
  pleaseSelectSystemImage: "请选择系统镜像",
  pleaseSelectMemorySpec: "请选择内存",
  pleaseSelectDiskSpec: "请选择磁盘",
  pleaseSelectBandwidthSpec: "请选择带宽",
  pleaseSelectCpuSpec: "请选择CPU",
  nodeNotSupportContainer: "该节点不支持容器类型",
  nodeContainerSlotsFull: "该节点容器槽位不足",
  autoSwitchToVM: "已自动切换到虚拟机类型",
  nodeNotSupportVM: "该节点不支持虚拟机类型",
  nodeVMSlotsFull: "该节点虚拟机槽位不足",
  autoSwitchToContainer: "已自动切换到容器类型",
  pleaseSelectProvider: "请先选择服务器",
  providerNotSupportType: "该服务器不支持所选实例类型或用户等级不足",
  createInstanceFailed: "创建实例失败",
  refreshedProviders: "已刷新，共 {count} 个可用服务器",
  noProvidersRetry: "当前没有可用的服务器节点，请稍后再试或联系管理员",
  instanceTypeNotSupported: "该服务器不支持所选实例类型或用户等级不足",
  instanceCreatedSuccess: "实例创建申请已提交，正在后台处理...",
  taskIdInfo: "任务ID: {taskId}，您可以在任务管理页面查看进度",
  duplicateTaskWarning: "您已有实例创建任务正在进行中，请稍后再试或查看任务页面",
  submitFailed: "提交申请失败，请稍后重试",
  requestTimeout: "请求超时，请稍后重试或查看任务页面",
  nodeResourceInsufficient: "该节点资源不足，请选择其他节点",
  networkConfig: {
    dedicatedIPv4: "独立IPv4",
    dedicatedIPv6: "独立IPv6",
    ipv6Only: "仅IPv6"
  },
  networkModes: {
    natIpv4: "NAT IPv4",
    natIpv4Ipv6: "NAT IPv4 + IPv6",
    dedicatedIpv4: "独立 IPv4",
    dedicatedIpv4Ipv6: "独立 IPv4 + IPv6",
    ipv6Only: "仅 IPv6",
    noPortMapping: "无端口映射"
  },
  resetConfig: "重置配置",
  confirmDialogTitle: "确认申请信息",
  confirmDialogMessage: "请确认以下配置信息是否正确：",
  confirmProvider: "服务器节点",
  confirmInstanceType: "实例类型",
  confirmImage: "系统镜像",
  confirmCpu: "CPU规格",
  confirmMemory: "内存规格",
  confirmDisk: "磁盘规格",
  confirmBandwidth: "带宽规格",
  confirmDescription: "备注说明",
  confirmSubmit: "确认提交",
  confirmCancel: "取消",
  confirmWarning: "提交后将创建实例任务，请确认配置无误！",
  redeemCodeTitle: "兑换码兑换",
  redeemCodePlaceholder: "请输入兑换码",
  redeemCodeSubmit: "立即兑换",
  redeemCodeSuccess: "兑换成功，实例已绑定到您的账户",
  redeemCodeRequired: "请输入兑换码",
  redeemCodeOnlyTag: "仅兑换码",
  redeemCodeOnlyNotice: "以下节点已启用仅兑换码领取，请直接在此处输入兑换码，无需先选择节点。",
  redeemCodeOnlySelectedTitle: "{provider} 已启用仅兑换码领取",
  redeemCodeOnlySelectedDescription: "该节点的独立兑换入口已停用，请使用页面上方的常驻兑换码区域领取。",
  useGlobalRedeemArea: "请使用页面上方的常驻兑换码区域",
  redeemCodeError: "兑换失败",
  nodeOffline: "该节点当前离线，无法选择",
  nodeInsufficientResources: "该节点资源不足，无法创建新实例",
  imageChangeMemoryReset: "镜像类型变更，当前内存规格不符合最低要求，已自动选择合适的规格",
  imageChangeDiskReset: "镜像类型变更，当前磁盘规格不符合最低要求，已自动选择合适的规格",
  providerChangeDiskReset: "Provider变更，当前磁盘规格不符合镜像的最低要求，已自动重置",
  pageLoadFailed: "页面加载失败，请稍后重试",
  loadProvidersFailed: "获取提供商列表失败，请检查网络连接",
  viewHardwareReport: "查看检测报告",
  hardwareTestReport: "硬件测试报告",
  noHardwareReport: "暂无测试报告",
  // GPU 直通配置
  gpuPassthrough: "GPU 直通",
  gpuEnabled: "启用 GPU 直通",
  gpuDeviceIds: "GPU 设备 ID",
  gpuDeviceIdsPlaceholder: "留空则附加所有GPU，多个ID用逗号分隔（如 0,1）",
  gpuDeviceIdsHint: "支持 LXD/Incus 与 Docker/Podman/Containerd/Orbstack 容器实例。留空将尝试附加节点上所有可用 GPU",
  gpuSelectAll: "全选",
  gpuDeselectAll: "取消全选",
  loadLimitsFailed: "获取用户限制失败",
  loadConfigFailed: "获取实例配置失败",
  loadImagesFailed: "获取镜像列表失败",
  loadGPUsFailed: "获取GPU列表失败"
}
